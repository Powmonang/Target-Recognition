...
import com.defectscan.utils.UploadImgUtil;
...
public class UploadController {
    ...
    public Result<List<String>> uploadImages_local(@RequestParam("images") List<MultipartFile> images) throws IOException {
        ...
        if (images == null || images.isEmpty()) {
            ...
            for (MultipartFile image : images) {
                try {
                    // 将图片存入本地临时文件夹
                    Path filePath = uploadImgUtil.uploadImgToTemp(image, tempDir);
    ...
    public Result sureUpLoad(@RequestBody UrlRequestDTO request) throws IOException {
        for (int i = 0; i < request.getUrl().size(); i++) {
            ...
            try {
                String imgTargetUrl = uploadImgUtil.uploadImgToLocal(tempUrl,tempDir,targetDir);
    ...
    public Result<List<String>> uploadImages_aliyun(@RequestParam("images") List<MultipartFile> images) throws IOException {
        ...
        try {
            ...
            for (MultipartFile image : images) {
                ...
                Path filePath = uploadImgUtil.uploadImgToTemp(image, tempDir);
                //截取至本地文件夹
                String imgTargetUrl = uploadImgUtil.uploadImgToLocal(filePath.toString(),tempDir,targetDir);
                ...
                String url = aliOssUtil.upload(image.getBytes(), fileName);
                ...
                // 将图片上传数据库
                // 定义源图片
                ImgOrigin imgOrigin = new ImgOrigin();
                imgOrigin.setLocalUrl(imgTargetUrl);
                imgOrigin.setAliyunUrl(url);
                imgOrigin.setImgName(fileName);
                imgOriginService.addImgOrigin(imgOrigin);  // 批量写入数据库
            }
